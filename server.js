...
app.get('/', (req,res)=>{
  ...
  const shortcutDownloadButtons = `<div class="quick-grid shortcut-download-grid"><a class="quick" href="https://www.icloud.com/shortcuts/03797100f137441c9580358f87a269cd" target="_blank">📶<span>Cài đặt WiFi</span></a><a class="quick" href="https://www.icloud.com/shortcuts/beeab292d1b143df923c9e217eae106f" target="_blank">🔄<span>Reset</span></a><a class="quick" href="https://www.icloud.com/shortcuts/e58a090763154e318317095f520a77fb" target="_blank">✏️<span>Đổi tên</span></a></div>`;
  let body = '';
  ...
  if (settings.showAccount) body += card('👤 Tài khoản', renderAccount(acc) + quickActions + shortcutDownloadButtons + renderWithdrawalMail(settings));
  else body += quickActions + shortcutDownloadButtons;
...
function normalizeSimStore(req){
  ...
  if (s.codeSimServiceId === undefined || String(s.codeSimServiceId || '').trim() === '' || ['tk','lf','tt'].includes(String(s.codeSimServiceId || '').trim().toLowerCase())) s.codeSimServiceId = '21';
...
function codeSimLooksLikeId(v){
  return /^\d+$/.test(String(v || '').trim());
}
async function resolveCodeSimServiceId(apiKey, serviceId){
  const raw = String(serviceId || '').trim();
  if (codeSimLooksLikeId(raw)) return raw;
  // Nếu lỡ lưu nhầm mã Grizzly như tk/lf/tt thì tự tìm dịch vụ TikTok thật của CodeSim.
  try {
    const list = await codesim('/service/get_service_by_api_key', { api_key: apiKey });
    if (codesimDataOK(list) && Array.isArray(list.json.data)) {
      const found = list.json.data.find(x => String(x.name || '').toLowerCase().includes('tiktok'));
      if (found && found.id !== undefined) return String(found.id);
    }
  } catch {}
  // Theo danh sách CodeSim bạn gửi, TikTok có id=21.
  return '21';
}
async function codesimGetNumber({ apiKey, serviceId, networkId, phone }){
  const realServiceId = await resolveCodeSimServiceId(apiKey, serviceId);
  const attempts = [];
  const makeParams = (extra = {}) => ({ api_key: apiKey, service_id: realServiceId, ...extra });

  // CodeSim chuẩn docs: chỉ bắt buộc api_key + service_id.
  // Tuyệt đối không dùng dữ liệu mẫu 0388 / {{sim_id}} và không tự nhảy sang service khác như Facebook.
  // Nếu người dùng chọn nhà mạng/đầu số thì chỉ thử sau lần tối giản để tránh lỗi 500 do network/phone không khớp.
  const plan = [{ label:'service_only', params: makeParams() }];
  if (networkId) plan.push({ label:'with_network', params: makeParams({ network_id: networkId }) });
  if (phone) plan.push({ label:'with_phone', params: makeParams({ phone }) });
  if (networkId && phone) plan.push({ label:'with_network_phone', params: makeParams({ network_id: networkId, phone }) });

  for (const item of plan) {
    const r = await codesim('/sim/get_sim', item.params);
    const detail = `${item.label} url=${r.url.replace(/api_key=[^&]+/,'api_key=***')} => ${codesimMessage(r)}`;
    ...
    if (codesimDataOK(r) && r.json.data && (r.json.data.otpId || r.json.data.idOtp) && r.json.data.phone) {
      ...
      return {
        ok:true,
        provider:'codesim',
        // id của phiên trên web = otpId để API /api/sim/status/:id kiểm tra code bằng otp_id.
        id:String(d.otpId || d.idOtp),
        otpId:String(d.otpId || d.idOtp),
        // simId chỉ dùng để hủy số: /sim/cancel_api_key/{simId}
        simId:String(d.simId || ''),
        number:String(d.phone),
        phone:String(d.phone),
        serviceUsed:String(d.serviceId || realServiceId),
        serviceName:d.serviceName || '',
        networkId:String(item.params.network_id || ''),
        cost:d.payment ?? '',
        raw:d,
        attempts
      };
  ...
  return {
    ok:false,
    raw:attempts.join(' | '),
    message:`CodeSim: Không thuê được số với service_id=${realServiceId}. Web đã gọi đúng docs /sim/get_sim?service_id=${realServiceId}&api_key=***. Nếu service_id=1 thuê được nhưng service_id=${realServiceId} lỗi, nghĩa là lỗi/hết số riêng dịch vụ đã chọn phía CodeSim, không được tự thuê nhầm dịch vụ khác.`
  };
}
...
function activeSimMeta(a, services, countries, prices, networks){
  if (String(a.provider || 'grizzly') === 'codesim') {
    const svcLabel = a.serviceName || serviceName(a.service, services || []);
    return `${esc(providerLabel('codesim'))} • ${esc(svcLabel)} • ${esc(networkName(a.networkId || '', networks || []))}`;
  }
...
async function simContext(req){
  ...
  // v87: luôn load/lưu được cả 2 API trước. Người dùng nhập Grizzly + CodeSim một lần,
  // sau đó chỉ chọn nhà cung cấp/dịch vụ cần thuê.
  let grizzlyBalance = s.apiKey ? 'Đang kiểm tra...' : 'Chưa nhập API key GrizzlySMS';
  let codeSimBalance = s.codeSimApiKey ? 'Đang kiểm tra...' : 'Chưa nhập API key CodeSim';
  let grizzlyServices = FALLBACK_SERVICES;
  let grizzlyCountries = FALLBACK_COUNTRIES;
  let grizzlyPrices = {};
  let codeSimServices = [];
  let codeSimNetworks = [['', 'Tất cả nhà mạng']];

  if (s.apiKey) {
    try { grizzlyBalance = parseBalance((await grizzly('getBalance', { api_key:s.apiKey })).text); } catch(e){ grizzlyBalance = 'Lỗi số dư GrizzlySMS: ' + e.message; }
    try { grizzlyServices = parseOptions(await grizzly('getServices', { api_key:s.apiKey }), FALLBACK_SERVICES); } catch {}
    try { grizzlyCountries = parseOptions(await grizzly('getCountries', { api_key:s.apiKey }), FALLBACK_COUNTRIES); } catch {}
    try { grizzlyPrices = parsePrices(await grizzly('getPrices', { api_key:s.apiKey, service:s.service }), s.service); } catch {}
  }
  grizzlyCountries = grizzlyCountries.filter(([id,label]) => isKnownCountryOption(id,label) || String(id) === String(s.country));

  if (s.codeSimApiKey) {
    try { codeSimBalance = parseCodeSimBalance(await codesim('/yourself/information-by-api-key', { api_key:s.codeSimApiKey })); } catch(e){ codeSimBalance = 'Lỗi số dư CodeSim: ' + e.message; }
    try { const got = parseCodeSimServices(await codesim('/service/get_service_by_api_key', { api_key:s.codeSimApiKey })); if (got.length) codeSimServices = got; } catch {}
    try { codeSimNetworks = parseCodeSimNetworks(await codesim('/network/get-network-by-api-key', { api_key:s.codeSimApiKey })); } catch {}
    if (!s.codeSimServiceId && codeSimServices.length) { s.codeSimServiceId = codeSimServices[0][0]; writeDomainSim(req, s); }
  }

  if (s.provider === 'codesim') {
    balance = codeSimBalance;
    services = codeSimServices;
    networks = codeSimNetworks;
  } else {
    balance = grizzlyBalance;
    services = grizzlyServices;
    countries = grizzlyCountries;
    prices = grizzlyPrices;
  }

  return { s, balance, services, countries, prices, networks, grizzlyBalance, codeSimBalance, grizzlyServices, grizzlyCountries, grizzlyPrices, codeSimServices, codeSimNetworks };
}
...
app.get('/thue-otp-sim', async (req,res)=>{
  const ctx = await simContext(req);
...
app.post('/thue-otp-sim/settings',(req,res)=>{
  ...
  writeDomainSim(req,{
    ...
    codeSimServiceId:req.body.codeSimServiceId || old.codeSimServiceId || '21',
...
app.get('/api/sim/status/:id', async (req,res)=>{
  ...
  if (sess && String(sess.provider || '') === 'codesim') {
    ...
    try{
      const r = await codesim('/otp/get_otp_by_phone_api_key', { api_key:s.codeSimApiKey, otp_id:sess.otpId || req.params.id });
